...
<script>
	...
	var nvehicle = <?php echo count($data); ?>
			...
			{
				showdialog();
				jQuery.post('<?php echo base_url(); ?>carmanagement/settenant', {id: v},
					function(r)
					{
						showdialog(r.html, "Set Rental");
					}
					, "json"
				);
			}
			
			function detailtenant(v)
			{
				showdialog();
				jQuery.post('<?php echo base_url(); ?>carmanagement/detail_tenant/', {id: v},
					function(r)
					{
						showdialog(r.html, "Detail Info Penyewa");
					}
					, "json"
				);
			}
	...
	<?php }  ?>				
		...
		//rentcar
		if (r.vehicle.customer)
		{
		var x 	= r.vehicle.customer.split(',');
		var xstar	= x[2].split('-');
		var xend	= x[3].split('-');
		var xlong	= x[1].split('-');
		
				jQuery("[id='customer"+vid+"']").html("<a href=" + "javascript:detailtenant(" + x[0] + ") >" + r.vehicle.tenant + "</a>" + "<br/>" + "Periode :" + " " + xlong + " " + "Days <br/>" + " " + xstar[2] + "-" + xstar[1] + "-" + xstar[0] + "<br/>" + "s/d" + "<br>" + " " + xend[2] + "-" + xend[1] + "-" + xend[0]);
			}
		//end rentcar	
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<?php } ?>
								<th width="15%" colspan="2"><a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?><?=$this->lang->line("lvehicle"); ?><?if ($sortby == 'vehicle_name') { echo '</u>'; }?></a></th>
								<!--rentcar-->
								<!--<th><?php echo "Driver"; ?></th>-->
								<th width="13%"><?php echo "Penyewa"; ?></th>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<!--rentcar-->
								</a><a href="javascript:vrequest(<?php echo $data[$i]->vehicle_id; ?>)"><font color='#0000FF'><?=$data[$i]->vehicle_no;?></font></td></a>
								...
								<!--<td>
                                    <span id="customer<?=$data[$i]->vehicle_id;?>" style=""></span>
								</td>-->
								<!--rentcar-->
								<td>
									<span id="customer<?=$data[$i]->vehicle_id;?>"></span>
									<!--<span id="customer <a href="javascript:vrequest(<?php echo $data[$i]->vehicle_id; ?>)"><font color='#FF0000'><?=$data[$i]->vehicle_no;?></font></a>-->
                                </td>
