		...
		<article class="container_12">
			<section class="grid_12">
				...
				<div class="block-border" id="istracking">
                  ...
                  <a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?><?=$this->lang->line("lvehicle"); ?><?if ($sortby == 'vehicle_name') { echo '</u>'; }?>
                  </a>
                </th>
								<th width="9%" align="center"><?=$this->lang->line("ldatetime"); ?></th>
                  ...
                  <div id="pointer<?=$data[$i]->vehicle_id?>" style="display: none; text-align: right;">&#9654;</div>
								...
								<?php if ($this->sess->user_group == 0) { ?>
								<td><a href="javascript:vform(<?php echo $data[$i]->vehicle_id; ?>)">
										<font color='#0000FF'><?=$data[$i]->vehicle_name;?></font><br />
									</a><?=$data[$i]->vehicle_no;?><br />
									<font color='red'><span id="driver_sj<?=$data[$i]->vehicle_id;?>"></span></font><br />
								</td>
								...
								<?php } else { ?>
									...
									<td width="1">
										<font color='#0000FF'><?=$data[$i]->vehicle_no;?></font><br />
										<font color='red'><span id="driver_sj<?=$data[$i]->vehicle_id;?>"></span></font><br />
											<?php if (in_array(strtoupper($data[$i]->vehicle_type), $this->config->item("vehicle_gtp"))) { ?>
												<div><?php echo $this->lang->line("lengine"); ?>
													<span id="engine<?=$data[$i]->vehicle_id;?>">-</span>
                ...
                <td>
                    <!--<span id="customer_groups<?=$data[$i]->vehicle_id;?>" ></span>-->
									<span id="no_sj<?=$data[$i]->vehicle_id;?>" style=""></span>
                </td>
								<!-- End Customer Groups -->
	...
	<?php }  ?>
		...
		else
		{
			...
			if (r.vehicle.since_geofence_in == "")
			{
				...
				//for iwatani
				if(r.vehicle.vehicle_user_id == "1837"){
		...
		//Get DRIVER SJ
		if (r.vehicle.driver_sj)
		{
			jQuery("[id='driver_sj"+vid+"']").html(r.vehicle.driver_sj);
		}

		//Get NO SJ
		if (r.vehicle.no_sj)
		{
			jQuery("[id='no_sj"+vid+"']").html(r.vehicle.no_sj);
		}
