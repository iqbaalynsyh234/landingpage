<script>
	...
	{
		jQuery.post("<?=base_url()?>transporter/customer/save", jQuery("#frmgroup").serialize(),
			function(r)
			{
				if (r.error)
				{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		<br />&nbsp;
		<?php if (isset($row)) { ?>
		<h1><?=$this->lang->line("lgroup_edit"); ?></h1>
		<?php } else { ?>
		<h1><?php echo "ADD Customer"; ?></h1>
		<?php } ?>
			<form id="frmgroup" onsubmit="javascript: return frmgroup_onsubmit(this)">				
    			...
    			<tr style="border: 0px;">
						...
						<td style="border: 0px;">
								...
								<input type="button" name="btncancel" id="btncancel" value=" Cancel " onclick="location='<?=base_url()?>group';" />
						</td>
					</tr>					
				</table>
			</form>		
