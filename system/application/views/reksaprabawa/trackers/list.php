...
<script>
	...
	var triggerCheckItem = false;
			...
			function destination_profile(v)
			{
				showdialog();
				jQuery.post('<?php echo base_url(); ?>destination/destination_info_detail/', {id: v},
					function(r)
					{
						showdialog(r.html, "No. CO detail");
					}
					, "json"
				);
			}
	...
	<?php }  ?>				
		...
		if (r.vehicle.destination)
        {
			...
			//jQuery("[id='destination"+vid+"']").html("<br/>" + "<a href=" + "javascript:destination_profile(" + sDes[0] + ")" + ">" + sDes[1] + "</a>");
			jQuery("[id='destination"+vid+"']").html("<br/>" + sDes[1]);
        }
		//end get destination
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<!--destination-->
								<td>
									<span id="destination<?=$data[$i]->vehicle_id;?>" style=""></span>
								</td>
								<!-- end desti -->
