...
<script>
	...
	{		
		...
		//Get destination
		var destrek = "";
		if (r.vehicle.destination)
        {
			var sDes = r.vehicle.destination.split('-');
			if (sDes[1])
			{
				destrek = sDes[1];
			}
	...
	<?php if ($ishowmap) { ?>	
		...
		addMarker(idx, (gidx+1) + ' ' + r.vehicle.vehicle_no, r.vehicle.gps.gps_longitude_real, r.vehicle.gps.gps_latitude_real, vid , r.vehicle.gps.car_icon, r.vehicle.vehicle_device_name, r.vehicle.vehicle_device_host, r, true, destrek);
	<?php }  ?>				
	...
	<?php if ($ishowmap) { ?>		
        ...
        function addMarker(idx, no, lng, lat, id, car, vename, vehost, r, isupdate, destrek)
        {
			var idx1;
			if (idx < 0)
			{
			...
			if (map) 
			{
                ...
                "<div id='pup2' onclick='javascript:showinfo("+idx1+")'>" + nos + "<br />" + 
				"<font color='white'><b>" + destrek + "</b></font></div>",
                kml_tracker5,
                false,
                null);
				var cr = false;
