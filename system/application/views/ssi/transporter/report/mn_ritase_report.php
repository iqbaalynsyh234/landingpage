<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>ssi_report/ritase_report", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>ssi_report/ritase_report_excel", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
        ...
        <h1>Ritase Report</h1>
			<table width="100%" cellpadding="3" class="tablelist" style="font-size: 12px;">
				...
				<tr id="filterdatestartend">
					<td width="10%">Date</td>
					<td>
						<input type="text" name="date" id="date" class="date-pick" value="<?=date('Y/m/d')?>" />
						to
						<input type="text" name="enddate" id="enddate" class="date-pick" value="<?=date('Y/m/d')?>" />
					</td>
				</tr>
				<tr><td>&nbsp;</td></tr>
				<tr>
