<script>
	...
	{
		jQuery("#loader").show();
		jQuery.post("<?=base_url()?>ssi_info_alert/info_alert_save", jQuery("#frmadd").serialize(),	
			function(r)
			{
				jQuery("#loader").hide();
				if (r.error)
	...
	<div id="main" style="margin: 20px;">
		...
		<?php } ?>
				<table width="90%" cellpadding="3" class="table sortable no-margin">
					<?php if (isset($row)) { ?>					
					<input type="hidden" id="id" name="id" value="<?=$row->info_alert_id;?>" />
					<tr style="display:none;">
						<td>ID</td>
						<td>:</td>
						<td><?=$row->info_alert_id;?></td>
					</tr>
					<?php } ?>
				<tr>
				...
				<tr>
					...
					<td>:</td>
					<td>
						<input name="info_alert_config_mobile" type="radio" value="1" checked> Yes</input>
						<input name="info_alert_config_mobile" type="radio" value="0" > No</input>
					</td>
				</tr>
				...
				<tr>
					...
					<td>:</td>
					<td>
						<input name="info_alert_config_status" type="checkbox" value="1" checked> Active </option>
					</td>
				</tr>
    			...
    			<tr>
						...
						<td>
								...
								<input type="button" name="btncancel" id="btncancel" value=" Cancel " onclick="location='<?=base_url()?>ssi_info_alert';" />
