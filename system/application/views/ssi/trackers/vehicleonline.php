...
function frmsummary_onsubmit()
	{
		jQuery("#loader").show();
		//jQuery.post("<?=base_url();?>ssi_report/vehicleonline", jQuery("#frmsummary").serialize(),
		jQuery.post("<?=base_url();?>ssi_report/vehicleonline2", jQuery("#frmsummary").serialize(), //to cek data kendaraan ssi
		//jQuery.post("<?=base_url();?>ssi_report/vehicleonline3", jQuery("#frmsummary").serialize(), //to cek data kendaraan tag
			function(r)
			{
				jQuery("#loader").hide();
				if(r.success == true){
...
<table>
		...
		<select name="vehicle">
			...
			?>
				<option value="<?php echo $myvehicles[$i]->vehicle_device; ?>">
					<?php echo $myvehicles[$i]->vehicle_no." - ".$myvehicles[$i]->vehicle_name ;?>
				</option>
			<?php
				}
			?>-->
	...
	<tr>
		...
		<td>&nbsp;</td>
		<td>
			<input type="radio" name="type" id="type" value="1" checked /><b>Online</b>
			<input type="radio" name="type" id="type" value="0" /><b>Offline</b>
		</td>
	</tr>
	<!--<tr>
		<td>Date :</td>
		<td>&nbsp;</td>
		<td>
			<input size="15" value="<?=date('d-m-Y')?>" maxlength="10" type="text" name="date" id="date" class="date-pick" />
		</td>
	</tr>-->
	<!--<tr><td>&nbsp;</td></tr>
