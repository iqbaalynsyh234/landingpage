...
<script>
	...
	var triggerCheckItem = false;
			...
			{
				showdialog();
				jQuery.post('<?php echo base_url(); ?>ssi_info_alert/info_vehicle_comment/', {id: v},
					function(r)
					{
	...
	<?php if ($ishowmap) { ?>	
		...
		addMarker(idx, (gidx+1) + ' ' + r.vehicle.vehicle_no, r.vehicle.gps.gps_longitude_real, r.vehicle.gps.gps_latitude_real, vid , r.vehicle.gps.car_icon, r.vehicle.vehicle_device_name, r.vehicle.vehicle_device_host, r, true, speed);
	<?php }  ?>				
		...
		if (r.vehicle.status1)
		{
			...
			if (r.vehicle.since_geofence_in == "")
			{
				...
				if(r.vehicle.starton_ssi){
					...
					if(r.vehicle.gps.gps_speed_fmt == 0){
		...
		if (r.vehicle.team)
        {
			var sTeam = r.vehicle.team.split('|');
			jQuery("[id='team"+vid+"']").html( "- " + sTeam[1] + "<br />" + "- " +sTeam[2] + "<br />" + "- " + sTeam[3] );
        }
		...
		//Khusus SSI User ID = 1933 //speed alert
		
		
			if(r.vehicle.gps.gps_speed_fmt){
				if(r.vehicle.vehicle_maxspeed > 0){
					if (r.vehicle.gps.gps_speed_fmt >= r.vehicle.vehicle_maxspeed)
					{
						jQuery("[id='speed_iwatani"+vid+"']").html("<img width=25px height=25px align='center' title='Speed Alert' src=<?php echo base_url();?>assets/images/alert_kecepatan.png />");
					}
	...
	<?php if ($ishowmap) { ?>		
        ...
        function addMarker(idx, no, lng, lat, id, car, vename, vehost, r, isupdate, speed)
        {
        	var popup = null;
        	...
        	var gml_url = "<?=base_url()?>map/kmllastcoord/"+lng+"/"+lat+"/"+id+"/"+car+"/on1/"+speed+"/"+r.vehicle.gps.css_delay_index;
        ...
        function info(r)
        {
			...
			jQuery('#dialog').dialog('option', 'dialogClass', 'classdialog');
	...
	function vehicle_online_report()
	{
		showdialog();
		jQuery.post('<?php echo base_url(); ?>ssi_report/',{},
		function(r)
		{
			showdialog(r.html, "Vehicle Status Report");
		}
		, "json"
		);
	}
            ...
            <div id="control-bar" class="grey-bg clearfix">
                <div class="container_12">
			         ...
			         <div class="float-right">
						 <a href="javascript:vehicle_online_report()"" class="button blue">
							 Vehicle Status Report 
							 <?=$this->lang->line("llist_trackers"); ?> ( <?=count($data);?> ), 
							 Offline ( <?php if(isset($totalmerah)) { echo $totalmerah; } else { echo "0"; } ?> )
							 Online ( <?php if(isset($totalonline)) { echo ($totalonline+$totalkuning); } else { echo "0"; } ?> )
						 </a>
						 <a href="javascript:showmap(false)" class="button blue"><?=$this->lang->line('lshow_table');?></a>
						 <a href="javascript:showmap(true)" class="button blue"><?=$this->lang->line('lshow_map');?></a>
			...
			<section class="grid_12">
				<div class="block-border">
					<form class="block-content form" name="frmsearch" id="frmsearch" method="post" action="<?=base_url()?>trackers">
						<h1><?=$this->lang->line("llist_trackers"); ?> ( <?=count($data);?> )
						OFFLINE ( <?php if(isset($totalmerah)) { echo $totalmerah; } else { echo "0"; } ?> )
						ONLINE ( <?php if(isset($totalonline)) { echo ($totalonline+$totalkuning); } else { echo "0"; } ?> )</h1>
						<fieldset class="grey-bg required">
							...
							<table width="100%" cellpadding="3" class="tablelist">
								<tr>
									<td>
										<select id="field" name="field" onchange="javascript:field_onchange()">
											<?php if ($this->sess->user_type != 2 && $this->sess->user_type != 5) { ?>
											...
											<option value="gpsstatus"><?php echo "GPS Status";?></option>
										</select>						
										...
										<select id="company" name="company" onchange="javascript: loadgroup1()" style="display: none;">
											<?php 
												...
												{
											?>
											<option value="<?php echo $companies[$i]->company_id; ?>" <?php echo $selected; ?>><?php echo $companies[$i]->company_name; ?></option>
											<?php } ?>
										</select>
										...
										<select id="gpsstatus_select" name="gpsstatus_select" style="display: none;">	
											<option value="0">GPS Online</option>
											<option value="2">GPS Online (Kuning)</option>
											<option value="1">GPS Offline (Merah)</option>
										</select>
								...
								<tr>
									<td>
									<button type="button" class="red" onclick="javascript:vehicle_online_report()">Vehicle Status Report</button>
									</td>
								</tr>
								-->
							</table>
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<?php if (($this->sess->user_type != 2 && $this->sess->user_type != 5) || $this->sess->user_company) { ?>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<?php if (($this->sess->user_type == 1) || ($this->sess->user_group == 0 && $this->sess->user_id == 1933)) { ?>
								...
								<?php if ($this->sess->user_group == 0 && $this->sess->user_id == 1933) { ?>
								...
								<br /><span id="speed_iwatani<?=$data[$i]->vehicle_id;?>" style=""></span>
								</td>
								<?php } else { ?>
								<td width="1px;"><font color='#0000FF'><?=$data[$i]->vehicle_name;?></font></td>
								<td width="1">
								<font color='#0000FF'><?=$data[$i]->vehicle_no;?><br /></font>
								<?php if (in_array(strtoupper($data[$i]->vehicle_type), $this->config->item("vehicle_gtp"))) { ?>
								<div><?php echo $this->lang->line("lengine"); ?>
								<span id="engine<?=$data[$i]->vehicle_id;?>">-</span>
								...
								<td>
								   ...
								   <span id="team<?=$data[$i]->vehicle_id;?>" style=""></span>
								</td>
								...
								<!--	<div  style="position: absolute;"><?php echo $this->lang->line("lengine"); ?></div> <div style="position: relative; left : 50%;" id="engine<?=$data[$i]->vehicle_id;?>">-</div> -->
									...
									<?php if ($this->sess->user_group == 0 || $this->sess->user_id == 1933 || $this->sess->user_id == 2288) { ?>
