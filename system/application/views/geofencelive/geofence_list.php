<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>geofencelist/search_geofence/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				if (r.error) {
					alert(r.message);
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>geofencelist/delete_geofence/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
	...
	function edit(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>geofencelist/edit_geofence/' + id, {},
				function(r)
				{
					if (r.error)
					{
					...
					showdialog(r.html, 'Form Edit Geofence Name');
				}
				, "json"
			);			
		}
				...
				<tr>
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							<option value="geofence_name">Name</option>
						</select>
						...
						<input type="submit" placeholder="Keyword" value="<?=$this->lang->line("lsearch");?>" />
