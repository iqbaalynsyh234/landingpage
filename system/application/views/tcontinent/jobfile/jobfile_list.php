<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>tcont_jobfile/search_jobfile/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
	...
	function field_onchange()
	{
		...
		jQuery("#keyword").hide();
		jQuery("#transporter_job_status").hide();
		jQuery("#transporter_job_number").hide();
		jQuery("#transporter_job_vehicle_no").hide();
		jQuery("#transporter_job_date").hide();
		jQuery("#transporter_job_date").datepicker( "destroy" );
		
		switch(v)
		{
			case "transporter_job_status":
				jQuery("#transporter_job_status").show();
				break;
			case "transporter_job_date":
				jQuery("#transporter_job_date").datepicker(
					{
								dateFormat: 'dd-mm-yy'
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>tcont_jobfile/delete_jobfile/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
	...
	function set_delivered(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>tcont_jobfile/set_delivered/' + id, {},
				function(r)
				{
					if (r.error)
					{
	...
	function excel_onsubmit(){
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>tcont_jobfile/export_to_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				if(r.success == true){
					jQuery("#loader2").hide();
...
<?=$navigation;?>
	...
	<div id="main" style="margin: 20px;">
				...
				<tr>
					...
					<td>
						...
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="transporter_job_number">Job Number</option>
							<option value="transporter_job_po">PO Number</option>
							<option value="transporter_job_vehicle_no">Vehicle No</option>
							<option value="transporter_job_status">Status</option>
						 </select>
		...
		<br />
		[ <a href="<?=base_url();?>tcont_jobfile/add_jobfile"><font color="#0000ff"><?php echo "Add New Job" ?></font></a> ]
		</div>
		<div id="result"></div>
