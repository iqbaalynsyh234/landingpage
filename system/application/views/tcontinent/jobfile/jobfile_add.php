...
function frmsave_onsubmit()
	{
		jQuery("#loader").show();
		jQuery.post("<?=base_url()?>tcont_jobfile/save_jobfile", jQuery("#frmsave").serialize(),	
			function(r)
			{
				jQuery("#loader").hide();
				if (r.error)
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<input type="hidden" id="vehicle_device" name="vehicle_device" value="" />
		<input type="hidden" id="vehicle_name" name="vehicle_name" value="" />
		<input type="hidden" id="vehicle_no" name="vehicle_no" value="" />
		...
		<tr>
          ...
          <td>:</td>
		  <td>
          	<input type='text' name="client"  id="client" size="30" value="<?=isset($row) ? htmlspecialchars($row->transporter_job_client, ENT_QUOTES) : "";?>">
		  </td>
        </tr>
