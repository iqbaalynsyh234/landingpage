...
<script>
	...
	var triggerCheckItem = false;
			...
			//tcontinent
			function detailjobfile(v)
			{
				showdialog();
				jQuery.post('<?php echo base_url(); ?>tcont_jobfile/detail_jobfile/', {id: v},
					function(r)
					{
						showdialog(r.html, "Detail Jobfile");
					}
					, "json"
				);
			}
	...
	<?php }  ?>				
		...
		//tcontinent
		if (r.vehicle.jobfile)
        {
			//var sJobfile = r.vehicle.jobfile;
			var sJobfile = r.vehicle.jobfile.split('|');
			jQuery("[id='jobfile"+vid+"']").html("<a href=" + "javascript:detailjobfile(" + sJobfile[0] + ")" + ">" + sJobfile[1] + "</a>");
        }
		//end tcontinent	
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
                                ...
                                <?php if ($this->sess->user_group == 0) { ?>
                                <td>
									<span id="jobfile<?=$data[$i]->vehicle_id;?>" style=""></span>
