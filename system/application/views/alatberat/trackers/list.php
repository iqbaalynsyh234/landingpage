...
<script>
	...
	<?php }  ?>				
		...
		//Get Hourmeter
		if (r.vehicle.hourmeter)
		{
			var vdate1 = r.vehicle.vehicle_created_date.split(' ');
			var v1 = vdate1[0];
			var vdate = v1.split('-');
			...
			var vm = vdate[1];
			var vd = vdate[2];
			var vc = vdate[2]+"-"+vdate[1]+"-"+vdate[0];
			//var cdate = new Date(vdate2);
			//var v_create_date = cdate.toString('dddd, MMMM ,yyyy');
			jQuery("[id='hourmeter"+vid+"']").html("Since at :" + vc + "<br />" +  "Hourmeter :" + " " + "<b><font color="+"green"+">"+r.vehicle.hourmeter+"</font></b>");
		}
		//end get hourmeter
            ...
            <div id="control-bar" class="grey-bg clearfix">
                <div class="container_12">
			         <div class="float-left">
                        <label class="button">Heavy Equipment Management System &copy; www.lacak-mobil.com</label>
			         </div>
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<th width="8%" style="text-align:center;"><a href="#" onclick="javascript:order('user_name')"><?if ($sortby == 'user_name') { echo '<u>'; }?><?=$this->lang->line("lusername"); ?><?if ($sortby == 'user_name') { echo '</u>'; }?></a></th>
								<?php } ?>
								<th width="15%" colspan="2" style="text-align:center;"><a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?><?=$this->lang->line("lvehicle"); ?><?if ($sortby == 'vehicle_name') { echo '</u>'; }?></a></th>
								<th style="text-align:center;"><?php echo "Driver"; ?></th>
								<th width="10%" align="center" style="text-align:center;"><?=$this->lang->line("ldatetime"); ?></th>
								<th style="text-align:center;"><?=$this->lang->line("lposition"); ?></th>
                                
                                <?php if ($this->sess->user_group == 0) { ?>
                                <th style="text-align:center;">Customer / Project Location</th>
								<?php } ;?>
                                
                                <!--<th width="10%"><?=$this->lang->line("lcoordinate"); ?></th>-->
								<th width="12%" style="text-align:center;"><?php echo "Device"; ?></th>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<!--	<div  style="position: absolute;"><?php echo $this->lang->line("lengine"); ?></div> <div style="position: relative; left : 50%;" id="engine<?=$data[$i]->vehicle_id;?>">-</div> -->
									...
									<?php } ?>
                                    <span id="hourmeter<?=$data[$i]->vehicle_id;?>" ></span>
