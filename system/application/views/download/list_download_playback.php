...
<table width="100%" cellpadding="3" class="table sortable no-margin" style="margin: 3px;">
			...
			?>
				<tr <?=($i%2) ? "class='odd'" : "";?>>
					...
					<td>
                        <?php
                            if (isset($vehicle) && count($vehicle)>0)
                            {
                                ...
                                {
                                    if ($v->vehicle_id == $data[$i]->autoreport_vehicle_id)
                                    {
					...
					<td>
                        <a href="<?=base_url();?>download/download_playback/<?=$data[$i]->autoreport_id;?>"><img src="<?=base_url();?>assets/newfarrasindo/images/downarrow.png" border="0" alt="Download" title="Download"></a>
