...
jQuery(document).ready(
  ...
  function initialize(){
    ...
    console.log("obj : ", obj);
    var latlngcenter      = obj[0].trip_mileage_coordinate_end;
    var latlngcentersplit = latlngcenter.split(" ");

    map = new google.maps.Map(
     document.getElementById("map"), {
       center: new google.maps.LatLng(parseFloat(latlngcentersplit[0]), parseFloat(latlngcentersplit[1])),
       zoom: 12,
     ...
     for (x = 0; x < obj.length; x++) {
       if (obj[x].trip_mileage_engine == 0) {
         var geofencefix = "";
         var locationfix = "";
         if (obj[x].trip_mileage_geofence_end.length > 1) {
            geofencefix = "Geofence : "+obj[x].trip_mileage_geofence_end + "<br>";
         }
         locationfix = obj[x].trip_mileage_location_end;

         var position      = obj[x].trip_mileage_coordinate_end;
         var positionsplit = position.split(" ");
         marker = new google.maps.Marker({
           position: new google.maps.LatLng(parseFloat(positionsplit[0]), parseFloat(positionsplit[1])),
           label: "" + labelindex ,
           map: map
         });

         var string = "<div style='z-index: 1;'>" +
            ". <br>" +labelindex + ". " + geofencefix + locationfix + "<br>" + obj[x].trip_mileage_end_time
           "</div>";
...
<?php } ?>
			<header class="panel-heading panel-heading-blue">REPORT NEW</header>
                ...
                <a href="javascript:void(0);" id="btnshowmap" type="button" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect m-b-10 btn-circle btn-warning" onclick="btnshowhide(0);"><small>Show Map</small></a>
                <a href="javascript:void(0);" id="btnshowtable" type="button" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect m-b-10 btn-circle btn-success" style="display:none;" onclick="btnshowhide(1);"><small>Show Table</small></a>
							</div>
					...
					<td style="text-align:center;font-size:12px;">
						...
						<?php
							...
							$geofence_start_name = $data[$i]->trip_mileage_geofence_start;?>
								...
								<?php echo $data[$i]->trip_mileage_location_start;?>
						<?php } ?>
              <a href="http://www.google.com/maps/place/<?php echo $data[$i]->trip_mileage_coordinate_start;?>" target="_blank">
                <strong>
                  <font color="red">
                    <br />
                    <?php echo $data[$i]->trip_mileage_coordinate_start;?>
                  </font>
                </strong>
              </a>
            <br />
					...
					<td style="text-align:center;font-size:12px;">
						...
						<?php
							...
							$geofence_end_name = $data[$i]->trip_mileage_geofence_end;?>
								...
								<?php echo $data[$i]->trip_mileage_location_end;?>
						<?php } ?>
            <a href="http://www.google.com/maps/place/<?php echo $data[$i]->trip_mileage_coordinate_end;?>" target="_blank">
              <strong>
                <font color="red">
                  <br />
                  <?php echo $data[$i]->trip_mileage_coordinate_start;?>
