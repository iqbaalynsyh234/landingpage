          ...
          <label class="button">Intelligent Transportation System &copy; www.lacak-mobil.com</label>
	         ...
	         <div class="float-right">
					  ...
					  ?>
						...
						<?php } ?>
  						<label class="button"> Total Checked: <?=$total_auto;?> Vehicles, at <?=$last_checked;?></label>
  						<label class="button"> <a href="#" onclick="javascript:viewrekap(<?=$userid;?>)"> <font color="Green"> List Vehicles: Online ( <?=$total_p+$total_k;?> ), Offline ( <?=$total_m;?> ) </font></a></label>
					<?php } ?>
	...
	<span id='tblrealtime'>
		<form id="frmvehicle">
			<?php
				...
				{
				?>
  				<input type="hidden" name="devid[]" id="devid<?=$data[$i]->vehicle_id;?>" value="<?=$data[$i]->vehicle_id;?>" />
  				<input type="hidden" name="devname[]" id="devname<?=$data[$i]->vehicle_id;?>" value="<?=$data[$i]->vehicle_device_name;?>" />
  				<input type="hidden" name="devhost[]" id="devhost<?=$data[$i]->vehicle_id;?>" value="<?=$data[$i]->vehicle_device_host;?>" />
				<?php } ?>
		</form>
		...
		<article class="container_12">
			<section class="grid_12">
									...
									<td>
										<select id="field" name="field" onchange="javascript:field_onchange()">
											...
											<?php if($this->sess->user_group == 0) { ?>
  											<option value="location"><?php echo "Locations";?></option>
											...
											<!-- <option value="garasinya"><?php echo "Garasi";?></option> -->
										</select>
										...
										<!-- <select style="display: none;" name="listgarasinya" id="listgarasinya">
											<option value="GARANSI ROHAN">GARASI ROHAN</option>
										</select> -->
				...
				<div class="block-border" id="istracking">
					<h1 style="font-size: 12px;">Vehicle Tracking Summarys</h1>
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<?php } ?>
  								<th width="20%" colspan="2"><a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?><?=$this->lang->line("lvehicle"); ?><?if ($sortby == 'vehicle_name') { echo '</u>'; }?></a></th>
								<?php if (in_array(strtoupper($this->sess->user_id), $this->config->item("user_view_pto"))) { ?>
									<th>Project</th>
								<?php } ?>
								<th><?php echo "Drivers"; ?></th>
								...
								<th width="16%"><?php echo "Device"; ?></th>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								<td>
                  <a name="a<?=$data[$i]->vehicle_id;?>"></a>
                  <div id="pointer<?=$data[$i]->vehicle_id;?>" style="display: none; text-align: right;">&#9654;</div>
                </td>
								...
								<td>
                  <a href="<?php echo base_url(); ?>user/add/<?php echo $data[$i]->user_id; ?>" target="_blank">
    								<font color='#0000FF'><?=$data[$i]->user_name;?></font>
                  </a>
								<span id="branch<?=$data[$i]->vehicle_id;?>"></span>
								</td>
								<?php } else { ?>
								<td>
                  <font color='#0000FF'><?=$data[$i]->user_name;?><br /></font>
                </td>
								<?php } ?>
								...
								<?php if ($this->sess->user_group == 0) { ?>
								<td>
                  <a href="javascript:vform(<?php echo $data[$i]->vehicle_id; ?>)"><font color='#0000FF'><?=$data[$i]->vehicle_name;?></font><br/></a>
									<?=$data[$i]->vehicle_no;?><br/><br/>
									<small>Masa Aktif : <?php echo date("d-m-Y",strtotime($data[$i]->vehicle_active_date2)); ?></small>

								<!--kusus iwatani -->
								<?php if ($this->sess->user_id == "183711") { ?>
									<span id="speed_iwatani<?=$data[$i]->vehicle_id;?>" style=""></span>
								<?php } ?>
								</td>

								<td>
                  <!--<a href="javascript:vform(<?php echo $data[$i]->vehicle_id; ?>)"><font color='#0000FF'><?=$data[$i]->vehicle_no;?>-->
									...
									<div>
  									...
  									<?php } ?>
  									<!--end-->
  									<span id="workhour<?=$data[$i]->vehicle_id;?>"></span>
									</div>
									<?php } ?>
								...
								<?php if (in_array(strtoupper($this->sess->user_id), $this->config->item("user_view_pto"))) { ?>
								<td width='10%;'>
										<span id="dataproject<?=$data[$i]->vehicle_id;?>" style="">
											<?php if(isset($dataproject)){?>
												<?php for ($x=0; $x < sizeof($dataproject); $x++) {?>
													<?php if ($dataproject[$x]['project_vehicle_device'] == $data[$i]->vehicle_device) {?>
														<?php echo $dataproject[$x]['project_name'] ?> <br>
														<?php echo $dataproject[$x]['project_customer_name'] ?> <br>
														<?php echo date("d-m-Y H:i", strtotime($dataproject[$x]['project_startdate'])) ?> <br>
														<?php echo $dataproject[$x]['project_durationofwork'] ?>
													<?php } ?>
												<?php } ?>
											<?php } ?>
										</span>
                ...
                <td>
                  ...
                  <span id="position<?=$data[$i]->vehicle_id;?>" style=""></span><br />
  								...
  								<span id="sharelinkkpa<?=$data[$i]->vehicle_id;?>"></span><br />
                ...
                <!-- End Customer Groups -->
                  ...
                  <?php } ?>
								...
								<td style="text-align: left;" valign="top">
									...
									<?php if ($data[$i]->vehicle_type == "TJAM" || $data[$i]->vehicle_type == "AT5") { ?>
								...
								<!--	<div  style="position: absolute;"><?php echo $this->lang->line("lengine"); ?></div> <div style="position: relative; left : 50%;" id="engine<?=$data[$i]->vehicle_id;?>">-</div> -->
									...
									<div  id="cutpower<?=$data[$i]->vehicle_id;?>" style="position: absolute;"></div>
									<?php } ?>
								</td>
...
<script type="text/javascript">
  var alldevice = '<?php echo json_encode($data); ?>';
  var objdevice = JSON.parse(alldevice);
  var loop      = 0;
  var totalloop = objdevice.length-1;
	var glasttime = 0;
  // console.log("totalloop : ", totalloop);
  // console.log("objdevice : ", objdevice);

  setInterval(function updaterowdata() {
    if (loop > totalloop) {
      loop = 0;
    }

    if (loop > 0) {
      jQuery("#pointer"+objdevice[loop-1].vehicle_id).hide();
    }
    var device = objdevice[loop].vehicle_device;
		var vehicle_no = objdevice[loop].vehicle_no;
    console.log("vehicle_no for update : ", vehicle_no);

    jQuery.post("<?=base_url();?>map/lastinfo", {device: device, lasttime: 100},function(r){
      console.log("response : ", r.vehicle);
      var vid = r.vehicle.vehicle_id;
      jQuery("#pointer"+vid).show();

      if (! r.vehicle.gps){
  			jQuery("#pointer"+vid).hide();
  			if (glasttime == 0)
  			{
  				...
  				jQuery("[id='workhour"+vid+"']").html("");
  			}

  			var n = jQuery("#devname"+vid).val();
  			var h = jQuery("#devhost"+vid).val();
      ...
      jQuery("[id='timestamp"+r.vehicle.vehicle_device_name+"_"+r.vehicle.vehicle_device_host+"']").html(r.vehicle.gps.gps_timestampori);
  		...
  		var infoposition = '<a target="_blank" href="http://maps.google.com/maps?q='+r.vehicle.gps.gps_latitude_real_fmt+','+r.vehicle.gps.gps_longitude_real_fmt+'"><font color="#0000ff">'+r.vehicle.gps.georeverse.display_name+'</font></a>';
  		// SHARE LINK PT.KPA START
  		var getsharelink = '<a class="button blue" href="<?=base_url()?>share/location/'+r.vehicle.vehicle_device_name+'" target="_blank">Share Link</a>'
  		// SHARE LINK PT.KPA END
  		var useridkpa = '<?php echo $this->sess->user_id?>';

      if (useridkpa == 981) {
				jQuery("[id='sharelinkkpa"+vid+"']").html(getsharelink);
			}

      jQuery("[id='position"+vid+"']").html(infoposition);
      ...
      if (r.vehicle.gps.direction == 0)
  		...
  		//Get dataproect
  		if (r.vehicle.dataproject)
  		{
  			// console.log("ini disini");
  			// alert("success");
  			jQuery("[id='dataproject"+vid+"']").html(r.vehicle.dataproject);
  		}

  		if (r.vehicle.status1)
  		{
  			jQuery("[id='engine"+vid+"']").html("<img width=20px height=20px src=<?php echo base_url();?>assets/images/realtime.png title='Engine'/>"+" <?php echo $this->lang->line('lon'); ?>");
  		...
  		{
  			jQuery("[id='engine"+vid+"']").html("<img width=20px height=20px src=<?php echo base_url();?>assets/images/park.png title='Engine'/>"+" <?php echo $this->lang->line('loff'); ?>");
      ...
      if (r.vehicle.vehicle_type == "X3_DOOR" || r.vehicle.vehicle_type == "TK315DOOR_NEW" || r.vehicle.vehicle_type == "TK510DOOR" || r.vehicle.vehicle_type == "TK510CAMDOOR" || r.vehicle.vehicle_type == "GT08SDOOR" ||
  		    r.vehicle.vehicle_type == "GT08DOOR" || r.vehicle.vehicle_type == "GT08CAMDOOR")
  		{
  		...
  		if (r.vehicle.vehicle_type == "X3_PTO" || r.vehicle.vehicle_type == "GT08PTO" || r.vehicle.vehicle_type == "GT08SPTO")
  		{
  		...
  		if (r.vehicle.vehicle_type == "TK315FAN")
  		{
  		...
  		//RFID
  		if (r.vehicle.vehicle_type == "GT08SRFID" || r.vehicle.vehicle_type == "GT08SRFIDDOOR" || r.vehicle.vehicle_type == "GT08SRFIDPTO")
  		{
  			//Get driver RFID
  			if (r.vehicle.driver_rfid)
  			{
  				var sDriver = r.vehicle.driver_rfid.split('-');
  		...
  		//Get driver ID CARD
  		if (r.vehicle.driver_idcard)
          {
  			var sDriver = r.vehicle.driver_idcard.split('-');
  			jQuery("[id='driver"+vid+"']").html("<a href=" + "javascript:driver_profile(" + sDriver[0] + ")" + ">" + sDriver[1] + "</a>");
          }
  		...
  		jQuery("[id='speed"+vid+"']").html("<img width=20px height=20px src=<?php echo base_url();?>assets/images/speedometer.png title='Speed'/>"+" "+r.vehicle.gps.gps_speed_fmt+ " kph");
  		//workhour
  		if(r.vehicle.workhour)
  		{
  			jQuery("[id='workhour"+vid+"']").html(""+"<br /><img width=20px height=20px src=<?php echo base_url();?>assets/images/workhour.png title='Duration'/>"+r.vehicle.workhour+"");
  		}
      ...
      //Khusus Iwatani User ID = 1837
  		...
  		//farrasindo dimunculkan di semua kondisi GPS
  		if (r.vehicle.vehicle_user_id == "389")
  		{
  			if(r.vehicle.cutpower)
  			{
  				jQuery("[id='cutpower"+vid+"']").html("<b><font color='red'>Power Off : "+" "+r.vehicle.cutpower+"</font></b><br />");
  			}
  		}
      ...
      // INI UNTUK MENGGANTI BACKGROUND ROW START
      ...
      if (r.vehicle.gps.css_delay){
        ...
        // INI UNTUK MENGGANTI BACKGROUND ROW END
