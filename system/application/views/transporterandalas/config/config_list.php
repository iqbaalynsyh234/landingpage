<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>andalas_config/search/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>andalas_config/delete/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Config List (<span id="total"></span>)</h1>
				...
				<tr>
					...
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="customer_company_name">Config Name</option>
		...
		<br />
		<?php if(count($data) < 1) {	 ?>
				[ <a href="<?=base_url();?>andalas_config/add"><font color="#0000ff"><?=$this->lang->line("ladd"); ?></font></a> ]
		<?php } ?>	
		</div>
		<div id="result"></div>		
	</div>
