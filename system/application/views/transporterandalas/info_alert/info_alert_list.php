<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>andalas_info_alert/search_info_alert/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>andalas_info_alert/delete_info_alert/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Dispatcher List (<span id="total"></span>)</h1>
				...
				<tr>
					...
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="alert_dispatcher_name">Dispatcher Name</option>
							<option value="alert_dispatcher_mobile">Dispatcher Mobile</option>
							<option value="alert_dispatcher_email">Dispatcher Email</option>
						</select>
		...
		[ <a href="<?=base_url();?>andalas_info_alert/add_info_alert"><font color="#0000ff"><?=$this->lang->line("ladd"); ?></font></a> ]
		<?php } ?>	
		</div>
		<div id="result"></div>		
	</div>
