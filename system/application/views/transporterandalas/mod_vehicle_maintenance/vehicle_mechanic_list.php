<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>transporter/mod_vehicle_maintenance/search_mechanic/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function mechanic_edit(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>transporter/mod_vehicle_maintenance/mn_mechanic_edit/' + id, {},
				function(r)
				{
					if (r.error)
					{
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>transporter/mod_vehicle_maintenance/delete_mechanic/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Mechanic List (<span id="total"></span>)</h1>
		...
		<br />
		[ <a href="<?=base_url();?>transporter/mod_vehicle_maintenance/add_mechanic"><font color="#0000ff">Add Mechanic</font></a> ]
		</div>
		<div id="result"></div>		
	</div>
</div>
