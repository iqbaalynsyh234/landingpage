<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>droppoint/search_dataparent/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function add()
		{
			showdialog();
			jQuery.post('<?=base_url()?>droppoint/add_dataparent/', {},
				function(r)
				{
					if (r.error)
					{
	...
	function edit(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>droppoint/edit_dataparent/' + id, {},
				function(r)
				{
					if (r.error)
					{
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>droppoint/delete_dataparent/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
				...
				<tr>
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="parent_code">Code</option>
						</select>
