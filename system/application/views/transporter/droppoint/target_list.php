<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>droppoint/search_target/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function add()
		{
			showdialog();
			jQuery.post('<?=base_url()?>droppoint/add_target/', {},
				function(r)
				{
					if (r.error)
					{
					...
					showdialog(r.html, 'Form Add Target Time');
				}
				, "json"
			);			
		}
	...
	function edit(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>droppoint/edit_target/' + id, {},
				function(r)
				{
					if (r.error)
					{
					...
					showdialog(r.html, 'Form Edit Target Time');
				}
				, "json"
			);			
		}
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>droppoint/delete_target/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Target OTA List (<span id="total"></span>)</h1>
				...
				<tr>
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							<option value="droppoint_name">Droppoint Name</option>
							<option value="droppoint_code">Droppoint Code</option>
							<?php if($this->sess->user_id == 1032 || $this->sess->user_id == 2974){ ?>
							<option value="droppoint_distrep">Distrep</option>
							<?php } ?>
						</select>
	...
	<?php if ($this->sess->user_group == 0) { ?>
		[ <a href="#" onclick="javascript:add()"><strong>ADD NEW</strong></font></a> ] [ <a href="<?=base_url();?>droppoint/target_hist" target="_blank"><strong>Go To History</strong></font></a> ]
		<?php }?>
		<br />
		<div id="result"></div>
