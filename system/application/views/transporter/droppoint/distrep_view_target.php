<script>
	...
	function page(p)
	{
		...
		jQuery("#result").hide();
		jQuery.post("<?=base_url();?>target_perdistrep/search/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
	...
	function company_onchange(){
		...
		if(data_company == 0){
			...
			var site = "<?=base_url()?>target_perdistrep/get_parent_by_company/" + data_company;
		    jQuery.ajax({
		        url: site,
	...
	function parent_onchange(){
		...
		if(data_parent == 0){
			...
			var site = "<?=base_url()?>target_perdistrep/get_distrep_by_parent/" + data_parent;
		    jQuery.ajax({
		        url: site,
		        success: function(response){
					jQuery("#distrep").html("");
