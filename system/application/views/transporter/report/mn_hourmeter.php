<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>report/hourmeter_report/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		...
		jQuery.post("<?=base_url();?>report/hourmeter_report_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
        ...
        <h1>Hourmeter Report</h1>
			...
			<table width="100%" cellpadding="3" class="tablelist" style="font-size: 12px;">
				...
				<tr>
					<td>&nbsp;</td>
					<td>
						<a href="<?=base_url();?>report/mn_history_hourmeter" class="button">> <font color="red">HISTORY HOURMETER REPORT </font> <</a>
					</td>
				</tr>
				-->
			</table>
