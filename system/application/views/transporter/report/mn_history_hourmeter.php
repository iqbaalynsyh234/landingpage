<script>
	...
	jQuery(document).ready(
		function()
		{
			...
			maximumDate = new Date(minimumDate.getFullYear(), minimumDate.getDate() -1);
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>report/history_hourmeter_report/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function getMaxDate() 
		{
			...
			var maxRequest = -1;
			maxDate.setDate(maxDate.getDate() + maxRequest );
			return maxDate;
		}
	...
	function excel_onsubmit(){
		...
		jQuery.post("<?=base_url();?>report/history_hourmeter_report_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
...
<?=$navigation;?>
	...
	<div class="block-border">
        ...
        <h1>History Hourmeter Report</h1>
				...
				<tr>
					...
					<td>
						<select id="vehicle" name="vehicle">
							...
							<option value="<?php echo $vehicles[$i]->vehicle_no; ?>"><?php if ($this->sess->user_type != 2) { echo $vehicles[$i]->user_name." - "; } ?><?php echo $vehicles[$i]->vehicle_name; ?> - <?php echo $vehicles[$i]->vehicle_no; ?></option>
							<?php } ?>							
						</select>
					</td>
				</tr>
