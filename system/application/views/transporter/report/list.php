...
<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>report/tripmileage_report/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		...
		jQuery.post("<?=base_url();?>report/tripmileage_report_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
	...
	/* function kml_onsubmit(){
		jQuery("#loader2").show();
		
		jQuery.post("<?=base_url();?>report/tripmileage_report_kml2/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				jQuery("#result").html(r.html);	
	...
	function kml_onsubmit(){
		...
		jQuery.post("<?=base_url();?>report/tripmileage_report_kml2/");
		
	}
	
	function trip_sum_onsubmit(){
		jQuery("#loader2").show();
		
		jQuery.post("<?=base_url();?>report/tripmileage_report_summary/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				jQuery("#result").html(r.html);	
			...
			<h1>Trip Mileage Report</h1>
				...
				<table width="100%" cellpadding="3" class="tablelist" style="font-size: 12px;">
					...
					<tr>
						...
						<input class="btn_export" type="button" name="excel_sum" id="btnsearchreport_sum" value="Summary Trip Mileage" onclick="javascript:return trip_sum_onsubmit()" />
						<!--<input class="btn_export" type="button" name="kml" id="btn_kml_report" value="Export To KML" onclick="javascript:return kml_onsubmit()" />-->
						<img id="loader2" style="display: none;" src="<?php echo base_url();?>assets/images/ajax-loader.gif" />
						<!--input type="button" name="pdf" value="Export To PDF" onclick="javascript:return pdf_onsubmit()" /-->
						
						<!--<?php echo anchor('report/tripmileage_report_kml2/', 'Export Data KML');?>-->
