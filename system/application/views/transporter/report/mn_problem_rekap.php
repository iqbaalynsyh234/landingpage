<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>report/rekap_problem_report/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>report/inout_geofence_detail_report_excel", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
	...
	function toexcell()
	{
		jQuery("#tblresult").table2excel({
		exclude: ".noExl",
		name: "RekapProblem Report",
		filename: "RekapProblem",
		exclude_img: true,
		exclude_links: true,
		exclude_inputs: true
		});
        ...
        <h1>Rekap Problem Report</h1>
			...
			<table width="100%" cellpadding="3" class="tablelist" style="font-size: 12px;">
				<tr id="filterdatestartend">
					<td width="10%">Date</td>
					<td>
                        <input type="text" name="startdate" id="startdate" class="date-pick" />
                        s/d
