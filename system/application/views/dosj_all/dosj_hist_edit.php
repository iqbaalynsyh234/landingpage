<script>
	...
	{
		jQuery("#loader").show();
		jQuery.post("<?=base_url()?>transporter/dosj_all/saveedit_hist_do", jQuery("#frmdosj").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
				alert(r.message);
...
<div id="main_data">
		...
		<table width="100%" cellpadding="3" class="table sortable no-margin" style="margin: 3px;">
				...
				<tr>
					<td style="text-align:right">Delivery Status</td>
					<td>
						
						<select id="do_delivered_status" name="do_delivered_status">
							<option value="">--Select Delivery Status--</option>				
							<option value="1" <? if (( isset($data)) && $data->do_delivered_status == '1') { ?>selected<?php } ?>>Completed</option>
							<option value="0" <? if (( isset($data)) && $data->do_delivered_status == '0') { ?>selected<?php } ?>>On Process</option>
						</select>
					</td>
				</tr>
				<tr>
