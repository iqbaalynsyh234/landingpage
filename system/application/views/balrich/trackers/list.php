...
<script>
	...
	<?php }  ?>				
		...
		if (r.vehicle.fan)
		{
			<?php 
				//user Balrich
				if ($this->sess->user_id == "1032" || $this->sess->user_type == 1) 
				{ 
			?>
		...
		//Get Merk
		if (r.vehicle.v_detail)
        {
			jQuery("[id='merk"+vid+"']").html(r.vehicle.v_detail);
        }
		//end get Merk
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<th width="2%" rowspan="2" style="text-align:center"><?=$this->lang->line("lno"); ?></td>
								<?php if (($this->sess->user_type != 2) || $this->sess->user_company) { ?>
								<th width="8%" rowspan="2" style="text-align:center"><a href="#" onclick="javascript:order('user_name')"><?if ($sortby == 'user_name') { echo '<u>'; }?><?=$this->lang->line("lusername"); ?><?if ($sortby == 'user_name') { echo '</u>'; }?></a></th>
								<?php } ?>
								<th colspan="3" width="17%" style="text-align:center"><a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?>VEHICLE<?if ($sortby == 'vehicle_name') { echo '</u>'; }?></a></th>
								<th rowspan="2" style="text-align:center">Engine</th>
								<th rowspan="2" width="9%" style="text-align:center"><?=$this->lang->line("ldatetime"); ?></th>
								<th rowspan="2" style="text-align:center"><?=$this->lang->line("lposition"); ?></th>
								...
								<th width="12%" rowspan="2" style="text-align:center"><?php echo "Device"; ?></th>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<td><span id="merk<?=$data[$i]->vehicle_id;?>"></span></td>
								<td><font color='#0000FF'><?=$data[$i]->vehicle_name;?></font></td>
