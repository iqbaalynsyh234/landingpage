<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>balrich_report/inout_geofence_report/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		...
		jQuery.post("<?=base_url();?>balrich_report/inout_geofence_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
					jQuery("#frmresult").attr("src", r.filename);			
				}else{
					alert(r.errMsg);
				}	
			}
...
<?=$navigation;?>
	...
	<div class="block-border">
		...
		<br />
		<div id="result"></div>		
		<iframe id="frmresult" style="display:none;"></iframe>
        </div>
	</div>
</div>
