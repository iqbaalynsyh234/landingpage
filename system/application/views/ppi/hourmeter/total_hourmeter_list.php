<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>transporter/ppi_hourmeter/search_total_hourmeter/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
	...
	function delete_date(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>transporter/ppi_hourmeter/delete_total_hourmeter/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	...
	<div id="main" style="margin: 20px;">
		...
		<h1>Total Hourmeter List (<span id="total"></span>)</h1>
