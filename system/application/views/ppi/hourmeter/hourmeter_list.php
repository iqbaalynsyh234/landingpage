<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>transporter/ppi_hourmeter/search_hourmeter/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
	...
	function delete_date(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>transporter/ppi_hourmeter/delete_hourmeter/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
	...
	function excel_onsubmit(){
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>transporter/ppi_hourmeter/export_to_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				if(r.success == true){
					jQuery("#loader").hide();
...
<?=$navigation;?>
	...
	<div id="main" style="margin: 20px;">
		...
		<h1>Data Hourmeter List (<span id="total"></span>)</h1>
				...
				<tr>
					...
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							<option value="data_hm_vehicle_no">Vehicle No</option>
							<option value="data_hm_vehicle_name">Vehicle Name</option>
						</select>
