...
<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>transporter/tupperware/search_slcars/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	{
		showdialog();
		jQuery.post('<?php echo base_url(); ?>transporter/tupperware/slcars_edit/', {id: v},
		function(r)
		{
			showdialog(r.html, "Data Transporter Edit");
		}
		, "json"
		);
	}
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>transporter/tupperware/delete_slcars/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Data Transporter List (<span id="total"></span>)</h1>
                            ...
                            <tr>
                                <td>
                                    <select id="field" name="field" onchange="javascript:field_onchange()">
                                        <option value="slcars_code">SLCARS</option>
										<option value="slcars_name">Transporter</option>
                                    </select>
		...
		<br />
		[ <a href="<?=base_url();?>transporter/tupperware/add_slcars"><font color="#0000ff">Add</font></a> ]
		</div>
		<div id="result"></div>		
	</div>
</div>