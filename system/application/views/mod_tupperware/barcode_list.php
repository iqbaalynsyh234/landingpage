...
<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>transporter/tupperware/search_barcode_list/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>transporter/tupperware/delete_upload_barcode/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Barcode List ( From Upload ) (<span id="total"></span>)</h1>
                            ...
                            <tr>
                                <td>
                                    <select id="field" name="field" onchange="javascript:field_onchange()">
                                        <option value="transporter_barcode">Barcode No</option>
										<option value="transporter_barcode_slcars">SLCARS</option>
                                    </select>
		...
		<br />
		[ <a href="<?=base_url();?>transporter/tupperware/mn_barcode"><font color="#0000ff">Upload</font></a> ]
		</div>
		<div id="result"></div>		
	</div>
</div>