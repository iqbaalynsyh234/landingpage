...
<script>
	...
	<?php }  ?>				
		...
		if (r.vehicle.slcars)
		{
			var slcars_split = r.vehicle.slcars.split('|');
			var my_slcars = "";
			if (slcars_split.length > 1)
			{
				var tot_split = slcars_split.length;
				for (var i=0;i<tot_split;i++)
				{
					my_slcars += slcars_split[i];
				}
				jQuery("[id='slcars"+vid+"']").html(my_slcars);
			}
			else
			{
				jQuery("[id='slcars"+vid+"']").html(slcars_split[0]);
			}
		}
		
		if (r.vehicle.noso)
		{
			var sodr_s = r.vehicle.noso.split('|');
			var my_sodr = "";
			if (sodr_s.length > 1)
			{
				var tot_split = sodr_s.length;
				for (var i=0;i<tot_split;i++)
				{
					my_sodr += sodr_s[i];
					my_sodr += "<br />";
				}
				jQuery("[id='noso"+vid+"']").html(my_sodr);
			}
			else
			{
				jQuery("[id='noso"+vid+"']").html(sodr_s[0]);
			}
		}
		
		if (r.vehicle.nodr)
		{
			var sodr_s = r.vehicle.nodr.split('|');
			var my_sodr = "";
			if (sodr_s.length > 1)
			{
				var tot_split = sodr_s.length;
				...
				jQuery("[id='nodr"+vid+"']").html(my_sodr);
			}
			else
			{
				jQuery("[id='nodr"+vid+"']").html(sodr_s[0]);
			}
		}
		
		if (r.vehicle.dbcode)
		{
			var dbcode_s = r.vehicle.dbcode.split('|');
			var my_dbcode = "";
			if (dbcode_s.length > 1)
			{
				var tot_split = dbcode_s.length;
				for (var i=0;i<tot_split;i++)
				{
					my_dbcode += dbcode_s[i];
					my_dbcode += "<br />";
				}
				jQuery("[id='dbcode"+vid+"']").html(my_dbcode);
			}
			else
			{
				jQuery("[id='dbcode"+vid+"']").html(dbcode_s[0]);
			}
		}
            ...
            <div id="control-bar" class="grey-bg clearfix">
                <div class="container_12">
			         ...
			         <div class="float-right">
						<?php if(!isset($this->sess->dist_code)){?>
								...
								<tr>
									<td>
										<select id="field" name="field" onchange="javascript:field_onchange()">
											...
											<option value="vehicletrans">Transporter Vehicle</option>
											...
											<option value="id_booking"><?php echo "ID Booking";?></option>
											<option value="noso"><?php echo "No. SO";?></option>
											<option value="nodr"><?php echo "No. DR";?></option>
											<option value="booking_loading"><?php echo "Status Loading";?></option>
										</select>						
					...
					<table class="table sortable no-margin" cellspacing="0" width="100%">
						<thead>
							<tr>
								...
								<?php } ?>
								<th><a href="#" onclick="javascript:order('vehicle_name')"><?if ($sortby == 'vehicle_name') { echo '<u>'; }?><?=$this->lang->line("lvehicle"); ?><?if ($sortby == 'vehicle_name') { echo '</u>'; }?></a></th>
								...
								<th style="text-align:center">No. DR</th>
						...
						<tbody>
							...
							<tr <?=($i%2) ? "class='odd'" : "";?> id="tr<?=$data[$i]->vehicle_id;?>">
								...
								<td>
									<font color='#0000FF'>
										<?php 
											if (isset($this->sess->dist_code)){echo"Tupperware";}
											else{echo $data[$i]->user_name;}
										?>
										<br />
									</font>
								</td>
								...
								<?php } else { ?>
								<td width="1px;"><font color='#0000FF'><?=$data[$i]->vehicle_name;?></font>
                                ...
                                <td>
								<span id="datetime<?=$data[$i]->vehicle_id;?>"></span><br />
								...
								<span id="slcars<?=$data[$i]->vehicle_id;?>" ></span>
								</td>
								<td style="text-align:center">
								<span id="noso<?=$data[$i]->vehicle_id;?>" ></span>
								</td>
								<td style="text-align:center">
								<span id="nodr<?=$data[$i]->vehicle_id;?>" ></span>
								</td>
								<td style="text-align:center">
								<span id="dbcode<?=$data[$i]->vehicle_id;?>" style="font-size:10px;" ></span>
								</td>
                                </td>
