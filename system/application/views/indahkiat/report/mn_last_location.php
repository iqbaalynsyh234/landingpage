<script>
	...
	function page(p)
	{
		...
		jQuery("#loader").show();
		jQuery.post("<?=base_url();?>report/last_location/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function excel_onsubmit(){
		...
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>report/last_location_excel", jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
				if(r.success == true){
        ...
        <h1>Last Location Detail</h1>
			...
			<table width="100%" cellpadding="3" class="tablelist" style="font-size: 12px;">
				...
				<tr id="filterdatestartend">
					<td width="10%">Date</td>
					<td><input type="text" name="startdate" id="date" class="date-pick" />
					</td>
				</tr>
				<tr><td>&nbsp;</td></tr>
				<tr>
