<script>
	...
	function page(p)
	{
		...
		jQuery("#offset").val(p);
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>bgn_muatan/search_muatan/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader2").hide();
	...
	function field_onchange()
	{
		...
		jQuery("#keyword").hide();
		jQuery("#muatan_vehicle_no").hide();
		jQuery("#muatan_startdate").hide();
		jQuery("#muatan_startdate").datepicker( "destroy" );
		jQuery("#muatan_list").hide();
		...
		switch(v)
		{
			case "muatan_data":
				...
				break;
			case "muatan_startdate":
				jQuery("#muatan_startdate").datepicker(
					{
								dateFormat: 'dd-mm-yy'
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>bgn_muatan/delete_muatan/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
	...
	/* function set_delivered(id)
		{
			showdialog();
			jQuery.post('<?=base_url()?>bgn_muatan/set_delivered/' + id, {},
				function(r)
				{
					if (r.error)
					{
	...
	function excel_onsubmit(){
		jQuery("#loader2").show();
		jQuery.post("<?=base_url();?>bgn_muatan/export_to_excel/", jQuery("#frmsearch").serialize(),
			function(r)
			{
				if(r.success == true){
					jQuery("#loader2").hide();
...
<?=$navigation;?>
	...
	<div id="main" style="margin: 20px;">
		...
		<h1>Muatan List (<span id="total"></span>)</h1>
				...
				<tr>
					...
					<td>
						...
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="muatan_vehicle_no">Vehicle No</option>
							<option value="muatan_vehicle_name">Vehicle Name</option>
							<option value="muatan_dest">Destination</option>
							<option value="muatan_note">Notes</option>
							<!--<option value="muatan_data">Muatan</option>-->
						 </select>
						<!--<select id="transporter_job_status" name="transporter_job_status" style="display: none;">
							<option value="2">Delivered</option>
							<option value="1">On Going</option>
						</select>-->
						<select id="muatan_list" name="muatan_list" style="display: none;">
								<option value="" selected='selected'>--Jenis Muatan--</option>
								<?php 
									$cdatamuatan = count($rdatamuatan);

									for($i=0;$i<$cdatamuatan;$i++){
										echo "<option value='" . $rdatamuatan[$i]->muatan_data_id ."'>" . $rdatamuatan[$i]->muatan_data_name . "</option>";
									}
								?>
		...
		<br />
		[ <a href="<?=base_url();?>bgn_muatan/add_muatan"><font color="#0000ff"><?php echo "Add New" ?></font></a> ] [ <a href="<?=base_url();?>bgn_muatan/data"><font color="#0000ff"><?php echo "Manage" ?></font></a> ]
		</div>
		<div id="result"></div>
