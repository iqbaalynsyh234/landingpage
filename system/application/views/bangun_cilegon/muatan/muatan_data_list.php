<script>
	...
	function page(p)
	{
		...
		jQuery.post("<?=base_url();?>bgn_muatan/search_data/"+p, jQuery("#frmsearch").serialize(),
			function(r)
			{
				jQuery("#loader").hide();
	...
	function delete_data(id)
		{
			if (confirm("Are you sure delete this data?")) {
				jQuery.post('<?=base_url()?>bgn_muatan/delete_data/' + id, {}, function(r){
					if (r.error) {
						alert(r.message);
						return;
					}else{
...
<?=$navigation;?>
	<div id="main" style="margin: 20px;">
		...
		<h1>Jenis Muatan (<span id="total"></span>)</h1>
				...
				<tr>
					...
					<td>
						<select id="field" name="field" onchange="javascript:field_onchange()">
							...
							<option value="muatan_data_name">Nama Muatan</option>
							<option value="muatan_data_note">Notes</option>
		...
		[ <a href="<?=base_url();?>bgn_muatan/add_data"><font color="#0000ff"><?=$this->lang->line("ladd"); ?></font></a> ]
		<?php } ?>	
		</div>
		<div id="result"></div>		
	</div>
