...
class GPSModel_autosetting extends Model {
	var $earthRadius = 6371;
	var $fromsocket;
	var $datainfo;	
	
	function GPSModel_autosetting () 
	{				
		parent::Model();		
	...
	function GetLastInfo($name, $host, $georeverse=true, $row=false, $lasttime=0, $type="")
	{
		...
		if (! $row)
		{		
			$tables = $this->gpsmodel_autosetting->getTable($rowvehicle);
		...
		//t6 invalid conditon
		if (($rowvehicle->vehicle_type == "T6" || $rowvehicle->vehicle_type == "T7") && $row->gps_status == "V")
		{
			$tables = $this->gpsmodel_autosetting->getTable($rowvehicle);
			$this->db = $this->load->database($tables["dbname"], TRUE);
		...
		{
			$row->gps_last_update = sprintf("%s", $row->gps_time_fmt);
		}
		else
		{
			$row->gps_last_update = sprintf("%s %s", $row->gps_date_fmt, $row->gps_time_fmt);
		}
	...
	function GetLastInfo_autosetting($name, $host, $georeverse=true, $row=false, $lasttime=0, $type="")
	{
		$this->db = $this->load->database("default", TRUE);
...
*/    	    	
	...
	function getGPSData($vehicle)
	{
		...
		$service_port = $url[1];
		$address = $url[0];
		if($address == "192.168.1.102")
		{
			$address = "27.111.40.251";
		}
        ...
        function getGPSDataUDP($vehicle)
        {
		$json = json_decode($vehicle->vehicle_info);
		if (! isset($json->vehicle_ws)) return FALSE;
		...
		//socket_set_timeout($sock, 0,0);
		$input = "lastinfo|".$vehicle->vehicle_device."\n";
	...
	function getDatabase($vehicle)
	{
        ...
        function getDatabaseUDP($vehicle)
        {
                $json = json_decode($vehicle->vehicle_info);
                if (! isset($json->vehicle_ws)) return FALSE;
                ...
                $input = "database|".$vehicle->vehicle_device."\n";
