...
class Newmobile extends Base {
	...
	function getdevices_bycompany()
	{
		header('Access-Control-Allow-Origin:*');
		$userid = isset($_POST['iduser']) ? trim($_POST['iduser']) : "";
		$companyid = isset($_POST['idcom']) ? trim($_POST['idcom']) : "";
		$mv = array();
	...
	function getcompany_bycreator()
	{
		...
		if ($q->num_rows() == 0)
		{
			exit(json_encode(array("m"=>"Data Company Not Available !")));
			return;
		}
			
		$data = $q->result();
		exit(json_encode(array("company"=>$data)));
		return;
