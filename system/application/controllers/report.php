...
class Report extends Base {
	...
	function playback_report()
	{
		...
		//if($rowvehicle->vehicle_type != "GT06" && $rowvehicle->vehicle_type != "TJAM" && $rowvehicle->vehicle_type != "A13" && $rowvehicle->vehicle_type != "TK303" && $rowvehicle->vehicle_type != "TK309" && $rowvehicle->vehicle_type != "TK309PTO" && $rowvehicle->vehicle_type != "GT06PTO" && $rowvehicle->vehicle_type != "TK315" && $rowvehicle->vehicle_type != "TK309N" && $rowvehicle->vehicle_type != "TK315N" && $rowvehicle->vehicle_type != "TK315DOOR" && $rowvehicle->vehicle_type != "GT06N"){
		if (!in_array(strtoupper($rowvehicle->vehicle_type), $this->config->item("vehicle_others"))){
			$sdate = date("Y-m-d H:i:s", strtotime("-7 hour", strtotime($startdate . " " . $shour . ":00")));
			$edate = date("Y-m-d H:i:s", strtotime("-7 hour", strtotime($enddate ." " . $ehour . ":59")));
		}
		...
		else
		{
			...
			{
				$html = $this->load->view("transporter/report/list_result_playback_other_portable", $params, true);
			}
			else
			{
...
//		print_r(count($rows));exit;
	...
	function mn_door_status()
	{
		...
		$door_list = array("T5DOOR","X3_DOOR","TK315DOOR_NEW");
	...
	{
		$door_others = array("X3_DOOR","TK315DOOR_NEW");
	...
	function mn_pto_status()
	{
		...
		$door_list = array("T5PTO","X3_PTO","GT08PTO","GT08SPTO","TK510PTO");
		...
		$this->params["content"] = $this->load->view('transporter/report/mn_pto_status', $this->params, true);
		$this->load->view("templatesess", $this->params);
	}
	
	function ptostatus_report()
	{
		$door_others = array("X3_PTO");
		$type_new = array("GT08PTO","GT08SPTO","TK510PTO");
		...
		else
		{
			...
			foreach($rows as $obj){
				...
				{
					if (in_array(strtoupper($rowvehicle->vehicle_type), $type_new))
					{
						if ($obj->gps_cs == 53){ //Door ON
							if($door != "ON") {
								$no++;
		...
		$params['data'] = $data_report;
		$params['vehicle'] = $rowvehicle;
		$html = $this->load->view("transporter/report/list_result_pto_status", $params, true);
		$callback['error'] = false;
		$callback['html'] = $html;
		echo json_encode($callback);
		return;
