...
class Member extends Base {
	...
	function dologin($referer="", $lacakmobil="")
	{
		...
		if($trackersite == ""){
			...
			$trackersite = "localhost/transporter.abditrack.com";
		}
		...
		if ($trackersite)
		{
			while(1)
			{
				...
				if ($total == 0)
				{
					...
					//Redirect oto-track user
					if ($trackersite == "app.oto-track.com" && base_url() == "http://www.abditrack.com/")
					{
						echo json_encode(array("error"=>false, "redirect"=>base_url()."ototrack_contact/info"));
						exit;
					}
					else
					{
						...
						// var_dump($token.'-'.$row->user_id);die();
						// echo "<pre>";
						echo json_encode(array("error"=>false, "redirect"=>"https://".$trackersite."/home/login/".$token));
						exit;
					}
				}
			}
