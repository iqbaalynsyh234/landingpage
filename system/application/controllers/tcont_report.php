...
class Tcont_report extends Base {
	var $otherdb;
	
	function Tcont_report()
	{
		parent::Base();	
	...
	function tripmileage_report()
	{
		...
		else
		{
			...
			$table = sprintf("%s_gps", strtolower($rowvehicle->vehicle_device));
			$tableinfo = sprintf("%s_info", strtolower($rowvehicle->vehicle_device));
			$this->db2 = $this->load->database("gpshistory", TRUE);
		...
		for($i=0;$i<$trows;$i++){
			...
			if($nopol != $rowvehicle->vehicle_no){ //new vehicle
				if($on && $i!=0){ 
					...
					$data[$rowvehicle->vehicle_no][$trip_no-1]['end_geofence_location'] = $this->getGeofence_location($rows[$i]->gps_longitude, $rows[$i]->gps_ew, $rows[$i]->gps_latitude, $rows[$i]->gps_ns, $rows[$i]->gps_name);
	...
	function mn_history_trip_mileage()
	{
		...
		$rows = $q->result();
		$this->params["vehicles"] = $rows;
		$this->params["content"] = $this->load->view('tcontinent/report/mn_history_trip_mileage', $this->params, true);		
		$this->load->view("templatesess", $this->params);
	}
...
//		print_r(count($rows));exit;
	...
	function history_trip_mileage()
	{
		...
		$offset = isset($_POST['offset']) ? $_POST['offset'] : 0;
		$sortby = isset($_POST['sortby']) ? $_POST['sortby'] : "transporter_job_date";
		$orderby = isset($_POST['orderby']) ? $_POST['orderby'] : "asc";
		...
		$this->dbtransporter->where("transporter_job_vehicle_device", $vehicle);
		...
		$params['data_job'] = $rows_r;
		$params["offset"] = $offset;
		$html = $this->load->view("tcontinent/report/list_result_history_trip_mileage", $params, true);
		$callback['error'] = false;
		$callback['html'] = $html;
		echo json_encode($callback);
		return;
	...
	function history_trip_mileage_excel()
	{
		...
		// total duration
			...
			if ((isset($data)) && ($data)>0)
			{
				...
				for ($j=0;$j<count($data);$j++)
				{
					$dur = $data[$j]->trip_mileage_duration;
					$tot_dur = $tot_dur + $data[$j]->trip_mileage_duration;
		...
		//total duration
		//print_r($jam."".$menit);exit();
		$objPHPExcel->getActiveSheet()->SetCellValue('F3', 'Total Duration :');
		$objPHPExcel->getActiveSheet()->getStyle('F3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$objPHPExcel->getActiveSheet()->getStyle('F3')->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->SetCellValue('G3', $jam."".$menit);
	...
	function mn_door_status()
	{
		...
		$this->db->order_by("vehicle_name", "asc");
		$this->db->order_by("vehicle_no", "asc");
		$this->db->where("vehicle_info LIKE '%".$port."%'");		
		$this->db->where("vehicle_status <>", 3);
