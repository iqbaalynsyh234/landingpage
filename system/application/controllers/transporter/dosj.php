...
class Dosj extends Base {
	...
	function index($field="all", $keyword="all", $offset=0)
	{
		...
		$this->params["content"] = $this->load->view('dosj/list.php', $this->params, true);		
	...
	function search($field="all", $keyword="all", $offset=0)
	{
		...
		$q = $this->dbtransporter->get("dosj", 20, $offset);
		$rows = $q->result();
		...
		$this->params["customer"] = $customer;
		$this->params["title"] = "Manage DO / SJ";
		...
		$callback['html'] = $this->load->view("dosj/listresult.php", $this->params, true);
	...
	function add()
	{
		...
		$this->params['content'] = $this->load->view("dosj/add", $this->params, true);
		$this->load->view("templatesess", $this->params);
	}
	...
	$qdo = $this->dbtransporter->get('dosj');
	$rdo = $qdo->row();
	$cdo = count($rdo);
	if ($cdo > 0)
	{
		$callback['error'] = true;
		$callback['message'] = "Duplicate Do Number, Please Check Your DO Number !";
	...
	$this->dbtransporter->insert("dosj_delivered", $data_delivered);
	...
	$callback["redirect"] = base_url()."transporter/dosj";
	...
	function dosj_history()
	{
		...
		$this->dbtransporter->join("dosj","dosj_no = do_delivered_do_number", "left");
		$this->dbtransporter->limit(1);
		$q1 = $this->dbtransporter->get("dosj_delivered");
		$rows1 = $q1->row();
		//print_r($rows1);exit;
		
		$params["data1"] = $rows1;
		$params["data"] = $rows;
		$html = $this->load->view('dosj/dosj_delivered_history', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function mn_manage_do()
	{
		...
		$this->params["content"] = $this->load->view('dosj/list_manage_do.php', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	}
	...
	function cost()
	{
		...
		$this->params["content"] = $this->load->view('dosj/list_cost.php', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	}
	...
	function cost_add()
	{
		...
		$this->params['content'] = $this->load->view("dosj/add_cost", $this->params, true);
		$this->load->view("templatesess", $this->params);
	}
	...
	function cost_add_destination()
	{
		...
		$this->params['destination'] = $destination;
		$this->params["title"] = "Manage Cost - ADD";		
		$this->params['content'] = $this->load->view("dosj/add_cost_destination", $this->params, true);
		$this->load->view("templatesess", $this->params);
	}
	...
	function cost_edit()
	{
		...
		$html = $this->load->view('dosj/cost_edit', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function save_cost()
	{
		...
		$callback["redirect"] = base_url()."transporter/dosj/cost";
	...
	function save_destination()
	{
		...
		//Insert to table transporter_hist_driver
		$this->dbtransporter->insert("destination", $data);
	...
	function mn_driver_hist_dosj()
	{
		...
		$this->params["driver"] = $driver;
		$this->params["vehicle"] = $vehicle;
		$this->params["content"] = $this->load->view('dosj/list_driver_hist.php', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	}
	...
	function mn_ritase_driver()
	{
		...
		$this->params["driver"] = $driver;
		$this->params["content"] = $this->load->view('dosj/list_ritase_driver.php', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	}
	...
	function mn_driver_performance()
	{
		...
		$this->params["driver"] = $driver;
		$this->params["content"] = $this->load->view('dosj/list_driver_performance.php', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	}
	
	function mn_upload_dosj()
	{
		$this->params["content"] = $this->load->view('dosj/upload_form_dosj', $this->params, true);		
		$this->load->view("templatesess", $this->params);	
	...
	function search_manage_do()
	{
		...
		$this->dbtransporter->join("driver","driver_id = do_delivered_driver", "left");
		$this->dbtransporter->join("cost","cost_id = do_delivered_cost", "left");
		$q = $this->dbtransporter->get("dosj_delivered", 20, $offset);
		$rows = $q->result();
		...
		$callback['html'] = $this->load->view("dosj/listresult_manage_do.php", $this->params, true);
		$callback['total'] = $total;
	...
	function search_cost()
	{
		...
		$this->dbtransporter->join("destination","destination_id = cost_destination", "left");
		$q = $this->dbtransporter->get("cost", $this->config->item("limit_records"), $offset);
		$rows = $q->result();
		...
		$this->dbtransporter->where("cost_company", $my_company);
		$this->dbtransporter->where("cost_status <> ", 2);
		$qtotal = $this->dbtransporter->get("cost");
		...
		$callback['html'] = $this->load->view("dosj/listresult_cost.php", $this->params, true);
		$callback['total'] = $total;
	...
	function search_hist_driver()
	{
		...
		$callback['html'] = $this->load->view("dosj/listresult_driver_hist.php", $this->params, true);
		$callback['total'] = $total;
	...
	function search_ritase_driver()
	{
        ...
        for($i=0;$i<count($rows);$i++)
        {
            if ($sdate != "" && $edate != "")
            {
                ...
                $qd = $this->dbtransporter->get("dosj_delivered");
                $rd = $qd->result();
		...
		$callback['html'] = $this->load->view("dosj/listresult_ritase_driver.php", $this->params, true);
	...
	function add_manage_do()
	{
		...
		$this->params["cost"] = $cost;
		$this->params["title"] = "Manage SO - ADD";		
		$this->params['content'] = $this->load->view("dosj/add_manage_do", $this->params, true);
		$this->load->view("templatesess", $this->params);
	}
	...
	$this->dbtransporter->where("do_delivered_company",$dosj_company);
	$qship = $this->dbtransporter->get("dosj_delivered");
	$rship = $qship->result();
	if (count($rship)>0)
	{
	...
	if ($tot_all == $tot_quantity)
	{
		...
		$this->dbtransporter->where('dosj_no',$dosj_no);
		$this->dbtransporter->update('dosj', $update_dosj);
	}
	...
	function dosj_this_day()
	{
		...
		$html = $this->load->view('dosj/dosj_this_day', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function dosj_edit()
	{
		...
		$html = $this->load->view('dosj/dosj_edit', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function dosj_hist_edit()
	{
		...
		$html = $this->load->view('dosj/dosj_hist_edit', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function dosj_delete()
	{
		...
		$html = $this->load->view('dosj/dosj_delete', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function dosj_hist_delete()
	{
		...
		$html = $this->load->view('dosj/dosj_hist_delete', $params, true);
		$callback["error"] = false;
		$callback["html"] = $html;
	...
	function delete_dosj()
	{
		...
		{
			$callback['error'] = true;
			$callback['message'] = "No SO"." ".$dosj_no.", masih te-Record di Data SO History, Proses Fail !";
			echo json_encode($callback);
			return;
		}
		else
		{
			$this->dbtransporter->where("dosj_no", $dosj_no);
			$this->dbtransporter->delete("dosj");
			$this->dbtransporter->close();
			$callback['error'] = false;
	...
	function delete_dosj_hist()
	{
		...
		$this->dbtransporter->where("do_delivered_company", $my_company);
		$this->dbtransporter->delete("dosj_delivered");
		
		//update status dosj jadi incomplete
		unset($status);
		$status["dosj_delivery_status"] = 0;
		$this->dbtransporter->where("dosj_no", $dosj_no);
		$this->dbtransporter->where("dosj_company", $my_company);
		$this->dbtransporter->update("dosj", $status);
		
		$callback['error'] = false;
		$callback['message'] = "Delete Record Complete";
		$callback["redirect"] = base_url()."transporter/dosj/mn_manage_do";
		echo json_encode($callback);
		return;
	...
	function saveedit_dosj()
	{
        ...
        */
		...
		/* if ($dosj_quantity_awal != $dosj_quantity)
		{
			...
			$this->dbtransporter->where("do_delivered_company", $my_company);
			$qdo_hist = $this->dbtransporter->get("dosj_delivered");
		...
		$this->dbtransporter->where('dosj_id', $dosj_id);
		$this->dbtransporter->update('dosj', $newdata);
		...
		if ($dosj_no_awal != $dosj_no)
		{
			...
			$this->dbtransporter->where('do_delivered_do_number', $dosj_no_awal);
			$this->dbtransporter->update('dosj_delivered', $new_do);
		}
		...
		if ($dosj_quantity_awal != $dosj_quantity || $dosj_quantity_mortar_awal != $dosj_quantity_mortar)
		{
			...
			$this->dbtransporter->where('do_delivered_do_number', $dosj_no_awal);
			$this->dbtransporter->update('dosj_delivered', $new_del);
		}
	...
	function saveedit_hist_do()
	{
		...
		if ($dev_date == ""):
			$callback['error'] = true;
			$callback['message'] = "Please Input Ship Date Delivery !";
			echo json_encode($callback);
			return;
		endif;
		...
		$this->dbtransporter->update('dosj_delivered', $new_data);
	...
	function driver_searchoverspeed()
	{
		...
		$this->dbtransporter->limit(1);
		$q_v = $this->dbtransporter->get("dosj_delivered");
		$r_v = $q_v->row();
		//print_r($r_v);exit;
