...
class Mod_vehicle_maintenance extends Base {
	...
	function vehicle_update()
	{
		...
		$data['mobil_engine_capacity'] = $this->input->post('mobil_year');
		$data['mobil_year'] = $this->input->post('mobil_engine_capacity');
	...
	function search_alert_service($field="all", $keyword="all", $offset=0)
	{
		$this->dbtransporter = $this->load->database('transporter', true);
		$my_company = $this->sess->user_company;
		...
		$q = $this->dbtransporter->get("service_alert", 20, $offset);
		$rows = $q->result();
