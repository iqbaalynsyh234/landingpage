...
class Sariroti extends Base {
	...
	function getota()
	{
		...
		$DateFrom_ex = explode("=", $parameter_ex[0]);
		$DateTo_ex = explode("=", $parameter_ex[1]);
		$PlantCode_ex = explode("=", $parameter_ex[2]);
		$Key_ex = explode("=", $parameter_ex[3]);
		
		$start = $DateFrom_ex[1];
		$end = $DateTo_ex[1];
		$plant = $PlantCode_ex[1];
		$key = $Key_ex[1];
		
		/* sudah ga bisa (24 07 2019)
		...
		*/
		...
		{
			print_r("Invalid Start Time");
			exit;
		}
		if(!isset($end) || $end=="")
		{
			print_r("Invalid End Time");
			exit;
		}
		if(!isset($plant) || $plant=="")
		{
			print_r("Invalid Plant Code");
			exit;
		}
		if(!isset($key) || $key=="")
		{
			print_r("NO DATA API KEY");
			exit;
		}
		
		if(!isset($key) || $key!="")
		{
			$this->dbtransporter->where("api_flag",0);
			$this->dbtransporter->where("api_active",1);
			$this->dbtransporter->where("api_key",$key);
			$q_key = $this->dbtransporter->get("droppoint_apikey");
			$r_key = $q_key->row();
			$total_key = count($r_key);
			if($total_key == 0){
				print_r("Invalid or expired API Key");
				exit;
			}
		...
		//select table ota report XML
		
		$this->dbreport->order_by("ID","desc");
		$this->dbreport->where("Creator",$userid);
		$this->dbreport->where("PlantCode",$plant);
		$this->dbreport->where("ReportDate >=",date("Y-m-d",strtotime($start)));
		$this->dbreport->where("ReportDate <=",date("Y-m-d", strtotime($end)));
		//$this->dbreport->limit(10);
		$q_xml = $this->dbreport->get($dbtable);
		$data_xml = $q_xml->result();
		$total_xml = count($data_xml);
		
		//total data ota
		if($q_xml->num_rows == 0)
		{
			print_r("No Data Droppoint or Data Not Available");
			exit;
		}
		
		
		//select ota xml berdasarkan tanggal
		for($i=0;$i<$total_xml;$i++){ 
				$value_ota = "";
				if($data_xml[$i]->TimeOTA == "NO DATA"){
					$value_ota = "NO DATA";
				}
				else if($data_xml[$i]->TimeOTA == "TIDAK ADA KIRIMAN"){
					$value_ota = "TIDAK ADA KIRIMAN";
				}
				else if($data_xml[$i]->TimeOTA == "-"){
					$value_ota = "-";
				}
				else{
					$value_ota = date("m-d-Y H:i",strtotime($data_xml[$i]->ReportDate." ".$data_xml[$i]->TimeOTA));
				}
				
				$ota = xml_add_child($ReportOTA, 'ota');
				xml_add_child($ota, 'PlantCode', $data_xml[$i]->PlantCode);
				xml_add_child($ota, 'PlantName', $data_xml[$i]->PlantName);
				xml_add_child($ota, 'OutletCode', $data_xml[$i]->OutletCode);
				xml_add_child($ota, 'OutletName', $data_xml[$i]->OutletName);
				xml_add_child($ota, 'DistrepCode', $data_xml[$i]->DistrepCode);
				xml_add_child($ota, 'DistrepName', $data_xml[$i]->DistrepName);
				xml_add_child($ota, 'SubOutletName', $data_xml[$i]->SubOutletName);
				xml_add_child($ota, 'SubOutletCode', $data_xml[$i]->SubOutletCode);
				xml_add_child($ota, 'Month', $data_xml[$i]->Month);
				xml_add_child($ota, 'Transporter', $data_xml[$i]->Transporter);
				xml_add_child($ota, 'Coordinat', $data_xml[$i]->Coordinat);
				xml_add_child($ota, 'KM', $data_xml[$i]->KM);
				xml_add_child($ota, 'TargetOTA', $data_xml[$i]->TargetOTA);
				xml_add_child($ota, 'TargetDate', $value_ota);
		}
		
		$output = xml_download($dom); 
