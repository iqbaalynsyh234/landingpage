...
class CI_Loader {
	...
	function database($params = '', $return = FALSE, $active_record = FALSE)
	{
		...
		if ($return === TRUE)
		{
			$connid = DB($params, $active_record);
			if ($connid == FALSE) 
			{
				return DB($CI->config->item("active_record"), $active_record);
			}
		...
		if ($CI->db->conn_id == FALSE)
		{
			...
			$CI->db =& DB($CI->config->item("active_record"), $active_record);
		}
	...
	function _ci_load($_ci_data)
	{
		...
		{
			$path = substr($_ci_path, 0, strlen($_ci_path)-strlen($_ci_file)-1);
			$paths = pathinfo($path);
			
			$_ci_path = $paths['dirname']."/".$_ci_file;
			if ( ! file_exists($_ci_path))
			{
